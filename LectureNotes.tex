...
\begin{proposition}\label{prop_priority}
	Die Laufzeiten der Operationen {\tt ExtractMax}, {\tt IncreaseKey} und {\tt Insert} sind $O(\log n)$.
\end{proposition}
\begin{proof}
...
\begin{proof}
	...
	\end{align}
	gilt.
	Die Ungleichung \eqref{eqthm_select5} gilt offenbar f\"ur $n\leq1000$, wenn wir $C'$ hinreichend gro\ss\ w\"ahlen.
...
\begin{lemma}\label{lemma_Hpm}
	Es gilt $\fH_{p,m}=p(p-1).$
\end{lemma}
\begin{proof}
	Wir m\"ussen zeigen, da\ss\ verschiedene $a,b$ verschiedene Funktionen $\cH_{a,b}$ ergeben.
\end{proof}
